...
export const AnnouncementsPage: React.FC<AnnouncementsPageProps> = ({ announcements, onBack }) => {
  ...
  return <div className="vop-ann-page">
    <div className="vop-ann-top">
      <button type="button" onClick={onBack} className="vop-ann-back"><ArrowLeft size={17}/> {t('common.back','Back')}</button>
      <div><span>VOP</span><h1>{t('navigation.announcements','Announcements')}</h1><p>{t('announcements.subtitle','Stay informed with the latest updates, opportunities and ministry news.')}</p></div>
    ...
    <section className="vop-ann-list">{list.map(item=><article key={item.id}>{item.imageUrl?<img src={item.imageUrl} alt="" />:<div className="vop-ann-placeholder"><Megaphone size={26}/></div>}<div className="vop-ann-list-copy"><div className="vop-ann-list-title"><h3>{item.title}</h3><ChevronRight size={18}/></div><p>{item.description}</p><footer><CalendarDays size={14}/><span>{item.tag || t('common.update','Update')}</span>{item.actionUrl&&<a href={item.actionUrl} target="_blank" rel="noreferrer">{item.actionText || t('common.open','Open')}</a>}</footer></div></article>)}{!list.length&&featured&&<div className="vop-ann-empty-inline">{t('announcements.no_matches','No other announcements match the current filter.')}</div>}</section>
  </div>;
};
