...
async function callSetup(body: Record<string, unknown>) {
  ...
  const response = await fetch('/api/vop-admin-setup', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify(body),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(typeof data.error === 'string' ? data.error : 'Administrator setup failed.');
  return data as { role?: string; message?: string; email?: string };
}
...
export function AdminSetupPage({ onBack }: { onBack: () => void }) {
  ...
  async function assignAdmin() {
    setBusy(true); setError(''); setMessage('');
    try {
      const data = await callSetup({ action: 'assign-admin', email, role: adminRole, adminNodeType: nodeType, adminNodeId: nodeId });
  ...
  return (
    <main style={{ maxWidth: '42rem', margin: '0 auto', padding: '1.5rem' }} aria-labelledby="admin-setup-title">
      ...
      <p>One-time security bootstrap and scoped administrator assignment. No browser storage is trusted for privileges.</p>

      {!role && (
        <section>
          <h2>Initialize Super Administrator</h2>
          <p>Sign in with the account that should become the permanent VOP super administrator, then enter the one-time setup secret supplied to you by the deployment owner.</p>
      ...
      {isSuperAdmin && (
        <section style={{ marginTop: '2rem' }}>
          ...
          <select id="vop-admin-role" value={adminRole} onChange={e => {
          ...
          <label htmlFor="vop-admin-node">Scope ID</label>
