...
export default function QuizLibrary(){
  ...
  return <div className="vop-reference-manager">
    <div className="vop-page-head"><div className="vop-heading"><div className="vop-heading-icon"><Share2 size={28}/></div><div><h1>{t('quiz.libraryTitle','Quiz Library')}</h1><p>{t('quiz.libraryDescription','Create reusable quizzes, publish them and share approved quizzes across organizations without transferring ownership.')}</p></div></div><div className="vop-reference-actions"><button className="vop-secondary" type="button" onClick={()=>void load()}><RefreshCw size={17}/>{t('common.refresh','Refresh')}</button><button className="vop-primary" type="button" onClick={()=>open(null)}><Plus size={17}/>{t('quiz.new','New Quiz')}</button></div></div>
    {error&&<div className="vop-alert error">{error}</div>}{message&&<div className="vop-alert success"><Check size={16}/>{message}</div>}
    {editorOpen ? <div className="vop-reference-editor"><div className="vop-section-title"><div><h2>{selected?t('quiz.edit','Edit Quiz'):t('quiz.new','New Quiz')}</h2><p>{t('quiz.ownerEditOnly','Only the owning organization and VOP Super Admin can edit a canonical quiz.')}</p></div><button className="vop-actions" type="button" onClick={()=>{setEditorOpen(false);setSelected(null)}}>×</button></div>
      <div className="vop-form-grid"><div className="vop-field"><label>{t('common.title','Title')} *</label><input value={title} onChange={e=>setTitle(e.target.value)}/></div><div className="vop-field"><label>{t('common.language','Language')} *</label><input value={language} onChange={e=>setLanguage(e.target.value)}/></div><div className="vop-field"><label>{t('sharing.permission','Sharing')}</label><select value={scope} onChange={e=>setScope(e.target.value as typeof scope)}><option value="private">{t('sharing.private','Private')}</option><option value="organization">{t('sharing.organizationOnly','Organization only')}</option><option value="shared">{t('sharing.shared','Shared')}</option></select></div><div className="vop-field"><label>{t('common.status','Publication')}</label><select value={published?'published':'draft'} onChange={e=>setPublished(e.target.value==='published')}><option value="draft">{t('common.draft','Draft')}</option><option value="published">{t('common.published','Published')}</option></select></div></div>
      ...
      <div style={{display:'grid',gap:12,marginTop:14}}>{questions.map((q,index)=><div className="vop-card vop-form-card" key={index}><div className="vop-section-title"><div><h3>{t('quiz.questionNumber','Question {current}',{current:index+1})}</h3></div><button className="vop-actions" type="button" onClick={()=>setQuestions(v=>v.filter((_,i)=>i!==index))}><Trash2 size={16}/></button></div><div className="vop-field"><label>{t('quiz.question','Question')}</label><textarea value={q.question} onChange={e=>setQuestions(v=>v.map((x,i)=>i===index?{...x,question:e.target.value}:x))}/></div><div className="vop-form-grid">{q.options.map((option,oi)=><div className="vop-field" key={oi}><label>{t('quiz.optionNumber','Option {current}',{current:oi+1})}</label><input value={option} onChange={e=>setQuestions(v=>v.map((x,i)=>i===index?{...x,options:x.options.map((o,j)=>j===oi?e.target.value:o)}:x))}/></div>)}</div><div className="vop-form-grid"><div className="vop-field"><label>{t('quiz.correctOption','Correct option')}</label><select value={q.answer} onChange={e=>setQuestions(v=>v.map((x,i)=>i===index?{...x,answer:Number(e.target.value)}:x))}>{q.options.map((_,oi)=><option key={oi} value={oi}>{t('quiz.optionNumber','Option {current}',{current:oi+1})}</option>)}</select></div><div className="vop-field"><label>{t('quiz.explanation','Explanation')}</label><input value={q.explanation||''} onChange={e=>setQuestions(v=>v.map((x,i)=>i===index?{...x,explanation:e.target.value}:x))}/></div></div></div>)}</div>
      <div className="vop-reference-editor-actions"><button className="vop-secondary" type="button" onClick={addQuestion}><Plus size={16}/>{t('quiz.addQuestion','Add Question')}</button><button className="vop-primary" type="button" disabled={saving} onClick={()=>void save()}><Save size={17}/>{t('common.save','Save Quiz')}</button></div>
    </div> : null}
    <div className="vop-reference-table-wrap">{loading?<div className="vop-empty">{t('quiz.loading','Loading quizzes…')}</div>:<table className="vop-reference-table"><thead><tr><th>{t('quiz.title','Quiz')}</th><th>{t('common.language','Language')}</th><th>{t('quiz.questions','Questions')}</th><th>{t('sharing.permission','Sharing')}</th><th>{t('common.status','Status')}</th><th>{t('common.actions','Actions')}</th></tr></thead><tbody>{items.map(item=><tr key={item.id}><td><strong>{item.title}</strong><div>{item.description||t('common.noDescription','No description')}</div></td><td>{item.language.toUpperCase()}</td><td>{item.questions?.length||0}</td><td>{item.sharingScope||'organization'}</td><td>{item.published?t('common.published','Published'):t('common.draft','Draft')}</td><td><div className="vop-reference-action-cell">{item.editable !== false && <button className="vop-actions" type="button" onClick={()=>open(item)} title={t('accessibility.edit','Edit')}><Edit3 size={16}/></button>}{item.sharingScope==='shared'&&item.published&&<button className="vop-actions" type="button" onClick={()=>void fork(item.id)} title={t('sharing.copyLink','Copy shared quiz')}><Copy size={16}/></button>}</div></td></tr>)}</tbody></table>}</div>
  </div>;
}
