...
export default function QuizLibrary(){
  ...
  const open=(quiz:Quiz|null)=>{
    setSelected(quiz);setTitle(quiz?.title||'');setDescription(quiz?.description||'');setLanguage(quiz?.language||'');setScope(quiz?.sharingScope||'organization');setPublished(quiz?.published===true);setQuestions(quiz?normalize(quiz.questions||[]):[]);
  };
  const save=async()=>{
    ...
    try{await quizApi('upsert',{id:selected?.id,data:{title:title.trim(),description:description.trim(),language:language.trim(),sharingScope:scope,published,questions:questions.map((q,i)=>({key:(selected?.id||'quiz')+'-q'+(i+1),question:q.question,options:q.options,correctOptionIndex:q.answer,explanation:q.explanation||''}))}});setMessage('Quiz saved.');open(null);await load();}
  ...
  return <div className="vop-reference-manager">
    ...
    {selected!==null || title ? <div className="vop-reference-editor"><div className="vop-section-title"><div><h2>{selected?'Edit Quiz':'New Quiz'}</h2><p>Only the owning organization and VOP Super Admin can edit a canonical quiz.</p></div><button className="vop-actions" type="button" onClick={()=>open(null)}>×</button></div>
