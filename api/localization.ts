...
export default async function handler(req: Request, res: Response) {
  try {
    ...
    if (action === 'bulkSave') {
      ...
      const status = ['draft','review','published'].includes(String(body.status||'draft')) ? String(body.status||'draft') : 'draft';
    ...
    const status = /Sign in/i.test(message) ? 401 : /permission|Only|membership|available|required/i.test(message) ? 403 : 400;
