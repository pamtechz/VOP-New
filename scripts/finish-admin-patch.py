...
if start < 0: raise SystemExit('candidate branch start marker not found')
end=s.find("\n        {activeTab===", start+10)
if end < 0: raise SystemExit('candidate branch end marker not found')
s=s[:start]+"{activeTab==='candidates'&&<CandidateEnrollment currentUser={currentUser}/>}"+s[end:]
